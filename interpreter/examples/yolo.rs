...
fn main() {
    ...
    let model = load_onnx(root.join("tiny-yolo-v3.onnx")).unwrap();
    let input_value = model.lookup_named_value("input_1").unwrap();
    let input_shape = model.lookup_named_value("image_shape").unwrap();
    ...
    let out = i.run(vec![
        ...
        (
            input_shape,
            Tensor::new(vec![1, 2].into(), vec![224f32, 224.0]),
        ),
    ]);
